...
class HQClient : private proxygen::HQSession::ConnectCallback {
 public:
	HQClient(
		const std::string &host, uint16_t port, const std::string &headers, const std::string &body,
		const std::string &path, const std::string &version, quic::TransportSettings transportSettings,
	...
	void start() {
		...
		quicClient_ = std::make_shared<quic::QuicClientTransport>(&evb_, std::move(sock));
		// TODO: turn on cert verification
		auto ctx = std::make_shared<fizz::client::FizzClientContext>();
		if (protocol_) {
			ctx->setSupportedAlpns({*protocol_});
		} else {
			ctx->setSupportedAlpns(
				{"h1q-fb", "h1q-fb-v2", proxygen::kH3FBCurrentDraft, proxygen::kH3CurrentDraft,
		...
		quicClient_->setCertificateVerifier(
			std::make_unique<proxygen::InsecureVerifierDangerousDoNotUseInProduction>());
		...
		if (!quicPskCache_) {
			quicPskCache_ = std::make_shared<proxygen::SynchronizedLruQuicPskCache>(1000);
		}
		...
		session_ = new proxygen::HQUpstreamSession(
			txnTimeout_, std::chrono::milliseconds(2000), nullptr, // controller
			tinfo, nullptr); // codecfiltercallback
 ...
 private:
		...
		if (version_.length() == 1) {
			...
			if (pos == std::string::npos) {
				auto errorMsg = folly::to<std::string>(
					"Invalid http-version string: ", version_, ", defaulting to HTTP 1.1");
				LOG(ERROR) << errorMsg;
			} else {
			...
			if (usePartialReliability_) {
				curls_.emplace_back(
					std::make_unique<PartiallyReliableCurlClient>(
						&evb_,
						(body_ == "" ? proxygen::HTTPMethod::GET : proxygen::HTTPMethod::POST),
						requestUrl,
						nullptr,
						headers_,
						body_,
	...
	void connectError(
		...
		auto errorMsg = folly::to<std::string>(
			"HQClient failed to connect, error=", toString(error.first));
		LOG(ERROR) << errorMsg;
	}
