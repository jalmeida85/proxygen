 ...
 */
class PartiallyReliableCurlClient : public CurlService::CurlClient, public folly::AsyncTimeout {
 ...
 public:
	PartiallyReliableCurlClient(
		...
		evb, httpMethod, url, proxy, headers, inputFilename, h2c, httpMajor, httpMinor, true, "", "", "-1"),
	...
	void onBodyPeek(
		uint64_t offset, const folly::IOBufQueue & /* chain */) noexcept override;

	void onBodySkipped(uint64_t offset) noexcept override;

	void onBodyRejected(uint64_t offset) noexcept override;

	void onBodyWithOffset(
		uint64_t bodyOffset, std::unique_ptr <folly::IOBuf> chain) noexcept override;
...
class PartiallyReliablePeerBase {
 public:
	explicit PartiallyReliablePeerBase(uint64_t chunkSize) : chunkSize_(chunkSize) {
	}

	void recordChunk(
		uint64_t chunkNum, uint64_t delayMs, std::unique_ptr <folly::IOBuf> data);
...
class PartiallyReliableSender : public PartiallyReliablePeerBase {
 public:
	explicit PartiallyReliableSender(uint64_t chunkSize) : PartiallyReliablePeerBase(chunkSize) {
