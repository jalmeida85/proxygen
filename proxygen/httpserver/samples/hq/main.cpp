...
DEFINE_string(path,
"/", "(HQClient) url-path to send the request to, or a comma separated list of paths to fetch in parallel");
...
"", "Path to the directory where qlog files will be written. File is called <CID>.qlog");
...
"0.0", "Latency");
DEFINE_string(plr,
"0.0", "Packet loss rate");
DEFINE_string(bytes,
"24262167", "Bytes requested");
...
int main(int argc, char *argv[]) {
	...
	if (FLAGS_draft_version != 0) {
		draftVersion = static_cast<quic::QuicVersion>(0xff000000 | FLAGS_draft_version);
	}
	quic::TransportSettings transportSettings;
	transportSettings.advertisedInitialConnectionWindowSize = FLAGS_conn_flow_control;
	// TODO FLAGS_stream*
	transportSettings.advertisedInitialBidiLocalStreamWindowSize = FLAGS_stream_flow_control;
	transportSettings.advertisedInitialBidiRemoteStreamWindowSize = FLAGS_stream_flow_control;
	transportSettings.advertisedInitialUniStreamWindowSize = FLAGS_stream_flow_control;
	transportSettings.defaultCongestionController = flagsToCongestionControlType(FLAGS_congestion);
	if (folly::to<uint16_t>(FLAGS_max_receive_packet_size) < quic::kDefaultUDPSendPacketLen) {
		LOG(ERROR) << "max_receive_packet_size needs to be at least " << quic::kDefaultUDPSendPacketLen;
		return -4;
	}
	transportSettings.maxRecvPacketSize = FLAGS_max_receive_packet_size;
	transportSettings.pacingEnabled = FLAGS_pacing;
	transportSettings.batchingMode = quic::getQuicBatchingMode(FLAGS_quic_batching_mode);
	...
	if (FLAGS_mode == "server") {
		...
		if (!FLAGS_psk_file.empty()) {
			auto pskCache = std::make_shared<proxygen::PersistentQuicPskCache>(
				FLAGS_psk_file,
				wangle::PersistentCacheConfig::Builder().setCapacity(1000).setSyncInterval(std::chrono::seconds(1)).build());
