...
CurlClient::CurlClient(
	...
	headers.forEach(
		[this](const string &header, const string &val) {
...
void CurlClient::initializeSsl(
	const string &caPath, const string &nextProtos, const string &certPath, const string &keyPath) {
	...
	folly::splitTo<string>(
		',', nextProtos, std::inserter(
			nextProtoList, nextProtoList.begin()));
...
void CurlClient::sslHandshakeFollowup(HTTPUpstreamSession *session) noexcept {
	...
	if (nextProto) {
		VLOG(1) << "Client selected next protocol " << string((const char *) nextProto, nextProtoLength);
	} else {
...
void CurlClient::printMessageImpl(
...
void CurlClient::connectError(const folly::AsyncSocketException &ex) {
	LOG_IF(ERROR, loggingEnabled_) << "Coudln't connect to " << url_.getHostAndPort() << ":" << ex.what();
}
...
void CurlClient::onEOM() noexcept {
	...
	stop_time_ = getTime();
	auto elapsed = stop_time_ - _start_time;
	float rate = bytes_ * 8.0f * 1000.0f / (1024.0f * 1024.0f * elapsed);
	LOG(INFO) << "latency: " << latency_ << "\tloss_percentage: " << packet_loss_ << "\t start: " << _start_time
